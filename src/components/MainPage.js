...
import ProfileModal from './ProfileModal'; // Ensure this is only imported once
...
const MainPage = () => {
  ...
  const [startDate, setStartDate] = useState(null); // Changed to null for date handling
  const [endDate, setEndDate] = useState(null); // Changed to null for date handling
  ...
  return (
    <div className="App">
      <header className="header">
        ...
        <div className="nav">
          <button onClick={() => setLibraryModalOpen(true)}>My Library</button>
          <button onClick={openChatbotModal}>Chatbot</button>
        </div>
